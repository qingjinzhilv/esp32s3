...
//遗嘱设置
const char *willMsg = "CLIENT-OFFLINE";    //遗嘱信息
const int willQos = 0;            //遗嘱Qos等级 可以用0 1   Qos越高等级也越高
const bool willRetain = true;    //遗嘱保留    false不保留  true保留   即在意外断开服务器时，并且服务器也意外关闭，服务器重新连接后还能收到相应的信息
...
void publishOnlioneStatus()
{
  ...
  {
    Serial.print("Publish Online Message: ");Serial.println(onlineMsg);
  }
  else
  {
