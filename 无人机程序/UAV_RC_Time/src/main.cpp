...
void Rocker_Send_Task(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    {
      My_RC_UAV_Data.throttle_value += 100;
    ...
    {
      if (My_RC_UAV_Data.throttle_value >= 199)
      {
        My_RC_UAV_Data.throttle_value -= 100;
      }
...
void loop()
{
  ...
  if (My_UAV_Data.UAV_Battery_Voltage == VBAT_LOW)
  {
    temp_1 = !temp_1;
    LED_Buzzer(Buzzer_Pin, temp_1);
  }
  else
    LED_Buzzer(Buzzer_Pin, 1);
  ...
  // 摇杆控制油门
  if (MyRockerData.rocker_Right_x <= 10 && MyRockerData.rocker_Right_y > 1990 && MyRockerData.rocker_Right_y < 2000)
  {
    char flag = THROTTLE_ADD;
    xQueueSend(xQueue_espnow_send_handle, &flag, 0);
  }
  else if (MyRockerData.rocker_Right_x > 4090 && MyRockerData.rocker_Right_y > 1990 && MyRockerData.rocker_Right_y < 2000)
  {
